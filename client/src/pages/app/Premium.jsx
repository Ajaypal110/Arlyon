...
const plans = [
  { name: 'Basic', price: '₹499', color: 'from-blue-500 to-cyan-500', icon: Zap, features: ['3 Super Likes/day', 'Advanced Filters', 'Read Receipts', '1 Boost/month'] },
  { name: 'Gold', price: '₹999', color: 'from-amber-500 to-orange-500', icon: Crown, popular: true, features: ['Unlimited Likes', 'See Who Liked You', '5 Super Likes/day', 'Advanced Filters', 'Read Receipts', '3 Boosts/month', 'Priority Matching'] },
  { name: 'Platinum', price: '₹1,999', color: 'from-primary to-secondary', icon: Star, features: ['Everything in Gold', '10 Super Likes/day', '5 Boosts/month', 'AI Date Assistant', 'Incognito Mode', 'Priority Support', 'Profile Highlights'] },
];
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            toast.success(`You are now a ${planName} member! 🎉`, { duration: 5000 });
        ...
        {plans.map((p, i) => (
          <motion.div key={p.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            ...
            <button 
              ...
              disabled={loadingPlan === p.name || (user?.isPremium && p.name !== 'Platinum')}
            ...
            >
              {loadingPlan === p.name ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
              {user?.isPremium ? (p.name === 'Platinum' ? 'Upgrade to Platinum' : 'Current Plan') : `Choose ${p.name}`}
            </button>
          </motion.div>
        ))}
      </div>
